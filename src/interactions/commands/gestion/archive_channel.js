...
module.exports = {
    ...
    runInteraction: async (_, interaction) => {
        ...
        const config = {};
        if (!client.configHandler.tryGet("archiveCategory", config)) {
            return interaction.reply({
                content: ":page_facing_up: La catégorie d'archive n'est pas configurée.",
                flags: [MessageFlags.Ephemeral]
            });
        }
        if (channel.parentId === config.value) return interaction.reply({
            content: ":page_facing_up: Ce salon est déjà archivé.",
            flags: [MessageFlags.Ephemeral]
        });
        await channel.setParent(config.value, {lockPermissions: true});
